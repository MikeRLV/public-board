...
const renderDescriptionWithLinks = (text: string, isEmbed = false) => {
  if (!text) return null;
    ...
    if (part.match(urlRegex)) {
      return (
        <a key={i} href={part} target="_blank" rel="noopener noreferrer"
          ...
          {isEmbed ? 'TICKET LINK' : 'TICKET LINK (EXTERNAL SITE, PROCEED WITH CAUTION)'}
        </a>
      );
    }
    return part;
...
export function DayDetailsModal({ activeDay, events, onClose, onVote, onPostClick, isEmbed = false }: any) {
                ...
                <div className="flex flex-col md:flex-row gap-8 md:gap-12 flex-1">
                    ...
                    <div className="flex flex-col gap-2 mb-4">
                      {e.source && (
                        <div style={{ borderColor: 'var(--primary)', color: 'var(--primary)', fontSize: 'calc(9px * var(--text-scale, 1))' }}
                          ...
                          Via {e.source === 'ticketmaster' ? 'Ticketmaster' : e.source === 'bandsintown' ? 'Bandsintown' : e.source === 'dice' ? 'Dice' : e.source === 'ntdlv' ? 'NTDLV' : e.source === 'vegas-underground' ? 'Vegas Underground' : e.source}
                        </div>
                      )}
                  ...
                  <div className="flex-1 flex flex-col w-full pt-4 md:pt-2">
                    <p style={scaled(15)} className="text-[var(--text-main)] opacity-80 leading-relaxed whitespace-pre-wrap font-sans">
                      {renderDescriptionWithLinks(e.description, isEmbed)}
                    </p>
                  </div>
                </div>
