...
export function DayDetailsModal({ activeDay, events, onClose, onVote, onPostClick }: any) {
  ...
  const handleAddTag = async (flyerId: string) => {
    ...
    if (!error) {
      ...
      onVote(); // Refresh parent to show new tag
    }
  };
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-neutral-900 w-full max-w-5xl h-[85vh] border border-white/20 rounded-xl flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
          {/* Post Invitation Section - Preserved logic */}
                ...
                <div className="flex flex-col md:flex-row gap-14">
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="space-y-4 mt-auto">
                      ...
                      {/* ADD TAG UI - Preserving sleek styling */}
                      <div className="flex gap-2 max-w-xs pt-4">
                        <input 
                          className="bg-neutral-900 border border-neutral-800 text-[10px] p-2 outline-none focus:border-yellow-500 flex-1 uppercase font-bold text-white rounded-sm"
                          placeholder="New Tag..."
                          value={tagInput[e.id] || ""}
