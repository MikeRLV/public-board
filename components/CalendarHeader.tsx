...
export function CalendarHeader({ currentDate, setCurrentDate, activeTowns = [], onMenuClick, setActiveTowns, ...props }: any) {
  ...
  // 2. Persist custom colors to LocalStorage
  useEffect(() => {
  ...
  // Shared Theme Switcher UI Component
  const ThemeSwitcher = ({ isMobile = false }) => (
    <div className={`flex flex-col gap-1.5 relative ${isMobile ? 'items-end' : ''}`} ref={isMobile ? themeLabRef : null}>
      ...
      {showThemeLab && (
        <div className={`absolute ${isMobile ? 'right-0' : 'left-full'} top-0 ${isMobile ? 'mt-8' : 'ml-3'} flex flex-col gap-3 bg-neutral-900 border border-white/10 p-3 rounded-sm shadow-2xl z-50 min-w-[160px] animate-in slide-in-from-${isMobile ? 'top' : 'left'}-2 duration-200`}>
          ...
          <div className="space-y-2">
            {[{ label: 'BG', key: 'bg' }, { label: 'PRI', key: 'primary' }, { label: 'BRD', key: 'border' }, { label: 'TXT', key: 'text' }, { label: 'ACC', key: 'muted' }].map((slot) => (
              <div key={slot.key} className="flex items-center gap-2 group">
                ...
                <input type="text" maxLength={7} value={customColors[slot.key as keyof typeof customColors]} onChange={(e) => handleColorChange(slot.key as keyof typeof customColors, e.target.value)} className="flex-1 min-w-0 bg-black border border-white/10 text-[10px] px-1.5 py-0.5 font-mono uppercase text-white outline-none focus:border-[var(--primary)] transition-colors" />
                <div className="relative w-4 h-4 shrink-0 overflow-hidden border border-white/20">
                  <input type="color" value={customColors[slot.key as keyof typeof customColors]} onChange={(e) => handleColorChange(slot.key as keyof typeof customColors, e.target.value)} className="absolute inset-0 w-8 h-8 -top-1 -left-1 opacity-0 cursor-pointer z-10" />
  ...
  return (
    <div className="grid grid-cols-3 items-center p-6 border-b border-white/10 bg-black sticky top-0 z-50">
      ...
      {/* 1. LEFT: Desktop Themes & Mobile Menu */}
      ...
      {/* 2. CENTER: Navigation - FIXED CLICKABILITY */}
      ...
      {/* 3. RIGHT: Mobile Themes & Branding */}
